...
ROOT_DIR = os.path.join(ROOT_DIR, "Mask_r_cnn")
print (ROOT_DIR)

MAMOGRAM_IMAGE_DIR = r"\scans\pseudo_color_image"  # Path of the mammograms
MAMOGRAM_MASK_DIR = r"\scans\preprocessed_mask1"  # Path of the ground truth masks
print(MAMOGRAM_IMAGE_DIR)
print(MAMOGRAM_MASK_DIR)
...
DEFAULT_LOGS_DIR = os.path.join(r"\your_path", "logs")  # Log directory for saving the weights
DEMO_SAVE_DIR = r"\Project\DetSegMaskRCNN\Mask_r_cnn\scans\seg_mask"  # path to save the segmentation masks
...
class MamogramDataset(utils.Dataset):
    ...
    def load_mamogram(self, subset):
        ...
        print('estou aqui 5')
        for fname in files:
            self.add_image("mamogram", image_id=fname, path=ROOT_DIR + MAMOGRAM_IMAGE_DIR + subset + "\\" + fname,
                           subset=subset, fname=fname)
            print(fname) # debug
...
def train(model):
    ...
    print('estou aqui 2')
    dataset_train.load_mamogram(r'\train')
    print('estou aqui 3') # NAO CHEGA AQUI
    dataset_train.prepare()

    # Validation dataset
    dataset_val = MamogramDataset()
    dataset_val.load_mamogram(r"\val")
    dataset_val.prepare()
...
def segment(model, imPath):
    ...
    for i in range(0, masks.shape[2]):
        ...
        file_name = DEMO_SAVE_DIR + "demo_mask_" + str(i) + "_" + fname + "_{:%Y%m%dT%H%M%S}.png".format(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', required=True,
                        metavar=r"\path\to\weights.h5",
                        help="Path to weights .h5 file or 'coco'")
    parser.add_argument('--image', required=False,
                        metavar=r'\path\to\image',
